...
def write_to_file(filename, instance, edu_features):
    f1 = open('output/'+filename, 'w')
    f2_name = 'output_enc/'+filename

    for idx in range(len (instance.edus)):
        for word in instance.edus[idx].words:
            f1.write(word+'|'+str(idx)+' ')
    f1.close()
    np.save(f2_name, edu_features.data.numpy())
...
def run_thread(files):
    ...
    for filepath in files:
        ...
        edu_features = rst.get_edu_representation(rst_data).view(len(instance.edus), -1)
        write_to_file(filename, instance, edu_features)
